...
def mixup_data(x, y, alpha=1.0, use_cuda=True):
    ...
    if use_cuda:
        ...
        index5 = torch.randperm(batch_size).cuda()
        index6 = torch.randperm(batch_size).cuda()
    ...
    if ind == 0:
        ...
        mixed_x = torch.cat([x, x[index1, :], x[index2], x[index3, :], x[index4, :], x[index5, :]], dim=1)
        mixed_y = y + y[index1, :] + y[index2, :] + y[index3, :] + y[index4, :] + y[index5, :]
    elif ind == 6:
        mixed_x = torch.cat([x, x[index1, :], x[index2], x[index3, :], x[index4, :], x[index5, :], x[index6, :]], dim=1)
        mixed_y = y + y[index1, :] + y[index2, :] + y[index3, :] + y[index4, :] + y[index5, :] + y[index6, :]
...
def main(fold):
    ...
    valid_fold.dropna(inplace=True)
    print(valid_fold.shape)
    ...
    if args.load_from:
        ...
        weights = torch.load("fold-0.bin")["model"]
